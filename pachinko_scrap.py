...
#         while True:
#             try:
#                 await page.goto("http://api.ipify.org?format=json", timeout=0)
...
#                 # country
#                 await page.goto("http://ip-api.com/json", timeout=0)
...
async def eachMachineFunc(page, model_name, scrap_days):
    ...
    try:
        scroll_div = await page.wait_for_selector('div.scroll', timeout=0)
...
async def eachModelFunc(page, full_url, model_name, filename, scrap_days):
    ...
    num_divs = len(divs)
    print(f"Found {num_divs} div elements inside target <td>.")
    ...
    for i in range(1, num_divs):
        ...
        try:
            target_td = await page.wait_for_selector(
                'td.nc-grid-color-fix.nc-text-align-center',
                timeout=0
            )
        except:
...
async def scrap_pachinko(context, shop_rows):
    """
    Main function to scrape pachinko data.
    """
    page = await context.new_page()
...
async def run():
    ...
    PROXY_USERNAME = "gzujsifm-JP-rotate"    # <- your JP rotating user
    PROXY_PASSWORD = "2cwmhvha2ian"          # <- your password

    print(f"プロキシ設定: {PROXY_SERVER}, ユーザー名: {PROXY_USERNAME}, パスワード: {PROXY_PASSWORD}")
    ...
    async with async_playwright() as p:
        ...
        try:
            await asyncio.gather(
                scrap_pachinko(context, shop_rows),
                # ip_logger_task(context),
            )
        finally:
            await context.close()
            await browser.close()
